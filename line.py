...
class LINE:
    def __init__(self, graph, embedding_size=8, negative_ratio=5, order='second',):
        ...
        self.graph = graph
        self.idx2node, self.node2idx = preprocess_nxgraph(graph) # 把节点编号与数组下标对应起来
        self.use_alias = True

        self.rep_size = embedding_size #
        self.order = order #  fisrt,second,or all
    ...
    def reset_training_config(self, batch_size, times):
        ...
        self.steps_per_epoch = (
            (self.samples_per_epoch - 1) // self.batch_size + 1)*times # //表示整数除法
    ...
    def _gen_sampling_table(self): # 函数功能是创建顶点负采样和边采样需要的采样表

        # create sampling table for vertex
        power = 0.75
        numNodes = self.node_size
        node_degree = np.zeros(numNodes)  # out degree [0,0,0,....]
        node2idx = self.node2idx

        for edge in self.graph.edges(): # 计算每个点的出度
            node_degree[node2idx[edge[0]]
                        ] += self.graph[edge[0]][edge[1]].get('weight', 1.0)

        total_sum = sum([math.pow(node_degree[i], power)
                         for i in range(numNodes)])             # Pn(v)
        norm_prob = [float(math.pow(node_degree[j], power)) /
                     total_sum for j in range(numNodes)]

        self.node_accept, self.node_alias = create_alias_table(norm_prob)
        # print('self.node_accept',self.node_accept)
        # print('self.node_alias',self.node_alias)
    ...
    def batch_iter(self, node2idx):
        # 这是一个生成器函数，生成器的输出应该为：
        # 一个形如（inputs，targets）的元组，
        # 一个形如（inputs, targets,sample_weight）的元组
    ...
    def train(self, batch_size=1024, epochs=1, initial_epoch=0, verbose=1, times=1):
        ...
        #  model.fit_generator ()这个方法指定相关参数用于训练模型
        self.hist = self.model.fit_generator(self.batch_it, epochs=epochs, initial_epoch=initial_epoch, steps_per_epoch=self.steps_per_epoch,
                                        verbose=verbose)
        # verbose：日志显示，0为不在标准输出流输出日志信息，1为输出进度条记录，2为每个epoch输出一行记录
        # print('batch_it',self.batch_it)
